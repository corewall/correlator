...
int tokenizeLine(char *lineStr, const string delimiter, vector<string> &tokens) {
	bool done = false;
	int pos = 0;
	...
	while (!done) {
		...
		int delimPos = line.find(delimiter, pos);
		if (delimPos != string::npos) {
			token = line.substr(pos, delimPos - pos);
			tokens.push_back(token);
			pos = delimPos + delimiter.length(); // skip past delimiter
		} else { // last token
			delimPos = line.find("\n", pos);
			if (delimPos != string::npos) {
				token = line.substr(pos, delimPos - pos);
...
int ReadAffineTable(FILE *fptr, Data* dataptr)
{
	...
	while (fgets(line, MAX_LINE, fptr) != NULL)
	{
		...
		tokenizeLine(line, delimiter, tokens);
		int tokenIndex = 0;

		string token = tokens[tokenIndex++];
		if (token[0] == '#' || lineCount == 1) // skip comment line, first line
			continue;
		...
		token = tokens[tokenIndex++]; // differential offset: unused
		token = tokens[tokenIndex++]; // growth rate: unused

		// "APPEND" is a type requested by IODP, but is really a TIE as far as Correlator is concerned.
		// Thus APPENDs are interpreted as AFFINE_TIEs.
		token = tokens[tokenIndex++]; // affine shift type
		int shiftType = AFFINE_NONE;
		if (strcmp(token.c_str(), AFFINE_TIE_STR) == 0 || (strcmp(token.c_str(), AFFINE_APPEND_STR) == 0))
		...
		newCore->setDepthOffset(cumOffset, value_type, applied, fromFile);
...
int ReadLegacyAffineTable(FILE *fptr, Data* dataptr)
{
	if(dataptr == NULL) return 0;
...
int WriteAffineTable(FILE *fptr, Data *dataptr)
{
	...
	// always write affine tables as CSV
	...
	for (int i=0; i < holesize; i++)
	{
		...
		for (int coreIdx=0; coreIdx < coresize; coreIdx++)
		{
			...
			const double diffOffset = (coreIdx == 0 ? depthOffset : depthOffset - prevCoreOffset);
			prevCoreOffset = depthOffset;
			...
			string dataType("");
			string comment("");
			if (coreTop == 0.0 && depthOffset == 0.0)
				shiftType = AFFINE_ANCHOR_STR;
			else if (depthOffset != 0.0) {
				// IODP requested an "APPEND" shift type to describe a core with the same
				// cumulative offset as the core above. APPEND ties can only result from the
				// "this core and all below" TIE action - the topmost core is a TIE, and all
				// below are APPENDs. However, as far as Correlator's concerned, they're really
				// all TIEs. Thus we'll write such shifts as APPENDs and convert them back to
				// TIEs when an IODP affine table is read.
				if (coreptr->getAffineType() == AFFINE_TIE)
					shiftType = (diffOffset == 0.0) ? AFFINE_APPEND_STR : AFFINE_TIE_STR;
			...
			makeDelimString(format, dataLine, site, holename, coreno, coretype, coreTop, shiftedTop, depthOffset, diffOffset, growthRate, shiftType.c_str(), dataType.c_str(), comment.c_str());
			dataLine += '\n';
...
// brg 1/11/2016: Write old-style (pre-2.0.1) affine table
int WriteLegacyAffineTable( FILE *fptr, Data* dataptr )
{
	...
	fprintf (fptr, "# Exp, Site, Hole, Core No, Section Type, Depth Offset, Y/N \n");
	fprintf (fptr, "# Generated By Correlator\n");
...
int CountSpliceTies(Data *dataptr)
{
    int count = 0;
    const int numHoles = dataptr->getNumOfHoles();
    for (int i = 0; i < numHoles; i++)
    {
    	...
    	{
    		Core *coreptr = holeptr->getCore(j);
    		if (coreptr == NULL) continue;
    		const int numTies = coreptr->getNumOfTies();
    		for (int k = 0; k < numTies; k++)
    		{
    			Tie *tieptr = coreptr->getTie(k);
...
int WriteSpliceIntervalTable(FILE *fptr, Data* dataptr)
{
	...
	string topSection("1");
	double topOffset = 0.0, topDepthMbsf = 0.0, topDepthMcd = 0.0;
	for (int tieIdx = 0; tieIdx < tieCount; tieIdx++)
	{
		Tie *tie = findTie(dataptr, tieIdx);
		if (tie && tie->getSharedFlag() == false)
		{
			TieInfo *fromInfo = tie->getInfoTied();
			...
			{
				cout << endl << "no to info" << endl;
				continue;
			}
			if (!tie->isAppend() && !toInfo->m_valueptr) continue;

			Value *toValue = toInfo->m_valueptr;
			topSection = toInfo->m_section[0];
			if (tie->isAppend() && !toValue)
			{
...
//		fprintf (fptr, "# AffineTable %s\n", affinefilename);
	...
	const int count = CountSpliceTies(dataptr);
	if (count == 0) return 0;
...
int WriteSplice( FILE *fptr,  Hole* dataptr, const char* leg, const char* site )
{
	...
	fprintf (fptr, "# Exp, Site, Hole, Core No, Section Type, Section No, top, bottom, total, raw data, smooth data, offset, depth, annotation\n");
	//fprintf (fptr, "# Generated By Correlator\n");
