...
class ImportLogDialog(ImportDialog):
	def __init__(self, parent, id, logPath, destSite):
		ImportDialog.__init__(self, parent, id)
		
		self.parent = parent
		self.logPath = logPath
		self.destSite = destSite
		self.importLabel = []  # related to "Unselect" option - retains old col names?
		self.selectedCol = -1
		self.logHole = "" # track hole of loaded log
		self.valid = False # does file's site/leg match destination site?
		
		self.SetTitle("Import Downhole Log Data")
	...
	def ColHeaderSelected(self, event):
		...
		wx.EVT_MENU(popupMenu, 14, self.OnLabelChanged)
		popupMenu.Append(1, "&Data")
		wx.EVT_MENU(popupMenu, 1, self.OnLabelChanged)
		popupMenu.Append(13, "&Unselect")
		wx.EVT_MENU(popupMenu, 13, self.OnLabelChanged)
		self.PopupMenu(popupMenu, event.GetPosition())
		
	def OnLabelChanged(self, event):
		opId = event.GetId()
		if opId == 13: # "Unselect" - reset label to original value
			origin_label = ""
			ith = 0 
	...
	def DoImport(self):
		...
		depthColumn = -1
		for i in range(cols):
			if self.sheet.GetColLabelValue(i) == "Data" :
				dataColumns.append(i)
			elif self.sheet.GetColLabelValue(i) == "Depth" :
				depthColumn = i 

		#print "[DEBUG] Depth number is " + str(depthColumn)
		if len(dataColumns) == 0 :
			glb.OnShowMessage("Error", "Please specify a Depth column and one or more Data columns", 1)
			return False

		datatype = ""
		ith_file = 0
		for dataColIndex in dataColumns :
			datatype = "?"
			ith = 0
			for label in self.importLabel :
				if ith == dataColIndex:
			...
			newLogNumber = dbu.GetNewLogNum(self.destSite.logTables, datatype)
			filename = self.destSite.name + '.' + datatype + '.' + newLogNumber +'.log.dat' 
			if datatype == '?' :
				filename = self.destSite.name + '.undefined' + '.log.dat' 

			outFile = dbu.GetSiteFilePath(self.destSite.name, filename)
			...
			fout.write(s)
			s = "# " + "Updated Time " + glb.GetTimestamp() + "\n"
			fout.write(s)
			s = "# " + "Site " + self.destSite.GetSite() + "\n"
			fout.write(s)
			s = "# " + "Leg " + self.destSite.GetLeg() + "\n"
			fout.write(s)
			s = "# " + "Hole " + self.logHole + "\n"
			fout.write(s)
			s = "# " + "Type " + datatype + "\n"
			...
			py_correlator.writeLogFile(glb.DBPath+'tmp/tmp.core', outFile, depthColumn, dataColIndex)

			glb.MainFrame.LOCK = 0
			py_correlator.openLogFile(outFile, dataColIndex)
			min, max = py_correlator.getRange("Log")
			min = int(100.0 * float(min)) / 100.0;
			max = int(100.0 * float(max)) / 100.0;
			
			outLog = model.DownholeLogTable(filename, self.logPath)
			outLog.min = min
			outLog.max = max
			outLog.dataType = datatype
			outLog.dataIndex = dataColIndex
			self.destSite.AddLog(outLog)
	...
	# helper logic for PopulateCells - compares a leg or site to a given value,
	...
	# read logfile and, if valid, pull values into first 120 rows of spreadsheet
	# and return True. If file doesn't match destination site/leg, return False.
	# (originally dbmanager.py OpenLOGFILE())
	def PopulateCells(self):
		leg, site = self.destSite.GetLegAndSite()
		...
		for line in f_obj :
			...
			if line.find(':') >= 0: # line with colon is a header/metadata line
				tokens = line.split()
				if tokens[0] == 'null' :
					continue
				type = tokens[0]
				...
				if type == "HOLE:" :
					...
					if not legSiteMatch:
						break
			elif line[0] == "#" : # header/metadata line
				...
				if modifiedLine[1] == "Leg" :
					...
					self.sheet.SetColLabelValue(0, "Depth")
					self.importLabel.append("Depth")  
					self.sheet.SetColLabelValue(1, modifiedLine[2])
					self.importLabel.append(modifiedLine[2])  
			else: # ordinary line of data
				...
				if header_flag == False :
					...
					for order in range(max_value) :
						self.sheet.SetCellValue(0, order, modifiedLine[order])
						s = s + str(modifiedLine[order]) + " "
						total_count += 1 
					s = s + str(modifiedLine[order]) + "\n"
					fout.write(s)
					row += 1
					self.sheet.SetColLabelValue(0, "?")
					if max_value > 9 :
						self.sheet.SetColLabelValue(8, "Depth")
						for i in range(8) :
					...
					if correlator_flag == False  :
						if row < 120 : 
							...
							for order in range(max_value) :
								if row < 120 :
									self.sheet.SetCellValue(row, column, modifiedLine[order])
								s = s + str(modifiedLine[order]) + " "
								count += 1 
								column += 1 
						...
						if row < 120 : 
							s = ""
							for order in range(max_value) :
								self.sheet.SetCellValue(row, order, modifiedLine[order])
								s = s + str(modifiedLine[order]) + " "
		...
		fout.close()
		if not legSiteMatch:
			glb.OnShowMessage("Error", "This log is not for " + leg + '-' + site, 1)
			return False
