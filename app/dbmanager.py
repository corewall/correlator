...
# 		self.dbPanel.SetBackgroundColour('white')
	...
	# options: dict with keys 'sectionLines', 'intervalLines', 'sectionNames' and boolean values
	...
	def ExportSplicedImage(self, sitePath, outFile, outPath, secSummFiles, spliceFile, options):
		...
		secsumm = SectionSummary.createWithFiles([os.path.join(sitePath, ssFile) for ssFile in secSummFiles])
		if not secsumm.hasColumn(SI.LEN_COLUMN):
			return False, "Section Summary files must have a '{}' column.".format(SI.LEN_COLUMN)

		sitDF = tabularImport.readSpliceIntervalTableFile(os.path.join(sitePath, spliceFile))
		imageDict = self._gatherSiteImages(sitePath)
		imagesToSplice = SI.prepareImagesForSplice(imageDict, secsumm, sitDF)
		...
		for holeDataList in HoleData:
			...
			for coreData in holeData[1:]:
				...
				for sectionIndex, sectionTop in enumerate(sections):
					...
					ssrow = SectionSummaryRow(hd_exp, hd_site, hd_hole, hd_core, 'Z', str(sectionIndex + 1), sectionTop, sectionBottom, row=None)
